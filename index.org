      ...
      (with-delayed-execution
        ...
        (when (autoload-if-found '(lsp lsp-deferred) "lsp-mode" nil t)
          ...
          (with-eval-after-load lsp-css
            (eval-and-compile
              (setq lsp-css-lint-float 'error)
              (setq lsp-css-lint-duplicate-properties 'error)
              (setq lsp-css-lint-empty-rules 'error)
              (setq lsp-css-lint-import-statement 'error)
              (setq lsp-css-lint-font-face-properties 'error)
              (setq lsp-css-lint-unknown-vendor-specific-properties 'error)))))
    #+END_SRC
*** dap-mode
    #+BEGIN_SRC emacs-lisp
      (eval-when-compile
