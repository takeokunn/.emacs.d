...
*** slime
    #+BEGIN_SRC emacs-lisp
      ...
      ;;   (autoload-if-found '(slime slime-setup) "slime" nil t)
...
#+begin_src emacs-lisp
  ...
  ;;   (autoload-if-found '(slime-company-maybe-enable) "slime-company" nil t)
  ...
  ;;     (add-hook slime-mode-hook #'slime-company-maybe-enable)
  ;;     (add-hook sldb-mode-hook #'slime-company-maybe-enable))
...
#+begin_src emacs-lisp
  ...
  (with-delayed-execution
    ...
    (when (autoload-if-found '(eshell-syntax-highlighting-global-mode) "eshell-syntax-highlighting" nil t)
...
#+begin_src emacs-lisp
  ...
  (with-delayed-execution
    ...
    (when (autoload-if-found '(exwm-enable) "exwm" nil t)
      (exwm-enable))

    (when (autoload-if-found '(exwm-randr-enable) "exwm-randr" nil t)
      (exwm-randr-enable))

    (when (autoload-if-found '(exwm-systemtray-enable) "exwm-systemtray" nil t)
      (exwm-systemtray-enable))

    (when (autoload-if-found '(exwm-cm-enable) "exwm-cm" nil t)
      (exwm-cm-enable))

    (when (autoload-if-found '(exwm-input-set-key) "exwm-input" nil t)
      (exwm-input-set-key (kbd "s-SPC") #'counsel-linux-app))

    (with-eval-after-load 'exwm-core
      (add-hook 'exwm-mode-hook #'display-battery-mode)
      (add-hook 'exwm-mode-hook #'display-time-mode))
