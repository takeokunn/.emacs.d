...
** font
#+begin_src emacs-lisp
  (defun my/set-fontset-font:around (set-fontset-font name target font-spec &optional frame add)
    "Warn if specified font is not installed."
    (if (stringp font-spec)
        (setq font-spec (font-spec :family font-spec)))
    (when (and (fontp font-spec)
               (find-font font-spec))
      (funcall set-fontset-font name target font-spec frame add)))

  (advice-add 'set-fontset-font :around #'my/set-fontset-font:around)

  (when (and window-system
             (member "Noto Sans" (font-family-list)))
    (add-to-list 'default-frame-alist '(font . "Noto Sans"))
    ...
    (set-fontset-font t '(#xfff9 . #xfffb) "Noto Sans Symbols2")
    ...
    (set-fontset-font t '(#x1ee00 . #x1eeff) "Noto Sans Math")
    ...
    (set-fontset-font t 'latin "Iposevka" nil 'append)
...
** keybind
   #+BEGIN_SRC emacs-lisp
     ...
     (global-set-key (kbd "C-x s") 'windmove-down)
     (global-set-key (kbd "C-x d") 'windmove-right)
     (global-set-key (kbd "C-x w") 'windmove-up)
...
#+begin_src emacs-lisp
  (when (autoload-if-found '(tab-bar-mode tab-bar-history-mode) "tab-bar" nil t)
    ...
    (with-eval-after-load 'projectile
      (defun my/set-tab-name-default (&rest _)
        (let ((proj-name (projectile-project-name)))
          (when proj-name
            (tab-bar-rename-tab proj-name))))
      (advice-add 'tab-bar-new-tab :after #'my/set-tab-name-default)
      (add-hook 'window-setup-hook #'my/set-tab-name-default)
